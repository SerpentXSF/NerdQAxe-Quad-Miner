...
{
    m_numPools = computeNumPools();

    for (int i = 0; i < m_numPools; i++) {
        m_stratumTasks[i] = nullptr;
        m_stratumConfig[i] = new StratumConfig(i);
    }
...
void StratumManager::task()
{
    ...
    // Create the Stratum + ping tasks for every configured pool
    for (int i = 0; i < m_numPools; i++) {
        char stratumName[24];
        char pingName[24];
        snprintf(stratumName, sizeof(stratumName), "stratum task %d", i);
        snprintf(pingName, sizeof(pingName), "ping task %d", i);

        m_stratumTasks[i] = createTask(i);
        xTaskCreatePSRAM(m_stratumTasks[i]->taskWrapper, stratumName, 8192,
                         (void *) m_stratumTasks[i], 5, NULL);

        m_pingTasks[i] = new PingTask(this, i);
        xTaskCreatePSRAM(m_pingTasks[i]->ping_task_wrapper, pingName, 4096,
                         (void *) m_pingTasks[i], 1, NULL);
    }

    if (m_poolmode == PoolMode::DUAL) {
        // dual: mine every configured pool at once
        for (int i = 0; i < m_numPools; i++) {
            connect(i);
        }
    } else {
...
void StratumManager::loadSettings(bool reconnect)
{
    ...
    bool requiresReconnect[MAX_POOLS];
    for (int i = 0; i < MAX_POOLS; i++) requiresReconnect[i] = reconnect;
...
void StratumManager::saveSettings(const JsonDocument &doc) {
    ...
    // Indexed multi-pool config: a "pools" array, one object per pool (array
...
{
    if (pool < 0 || pool >= MAX_POOLS) return;
...
void StratumManager::processCoinbase(int pool, const mining_notify *notify)
{
    ...
    if (pool < 0 || pool >= MAX_POOLS || !m_extranonce1[pool]) return;
