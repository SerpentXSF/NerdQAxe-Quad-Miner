...
#define NVS_CONFIG_POOL_WEIGHT "poolwt"   // indexed base: poolwt0..poolwt(MAX_POOLS-1)
...
namespace Config {
    ...
    // ---- Indexed multi-pool accessors (pool 0..MAX_POOLS-1) ----
    // Storage key = the legacy primary base name with the pool index appended,
    // e.g. "stratumurl0".."stratumurl3". A *missing* indexed key transparently
    // falls back to the legacy 2-pool keys: pool 0 -> primary ("stratumurl"),
    // pool 1 -> fallback ("fbstratumurl"); pools >= 2 use the plain default. So
    // existing 2-pool configs keep working with no migration write. Setters
    // always write the indexed key, making that pool authoritative from then on
    // (legacy keys are left untouched, safe for a firmware downgrade).
    // Implemented out-of-line in nvs_config.cpp. String getters allocate; free them.
    ...
    // Per-pool scheduling weight (percent; only the ratios among active pools
    // matter). Default 25 for every pool, so N configured pools split evenly.
