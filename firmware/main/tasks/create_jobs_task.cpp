...
// Global mining info instances - one per pool slot. Every slot must start
// pointing at its V1 instance: create_job_invalidate() dereferences
// miningInfo[pool] directly (no ensureV1 null-heal), so a null slot would crash
// if a pool disconnects before it ever received work.
static MiningInfoV1 s_miningInfoV1[MAX_POOLS];
MiningInfoBase* miningInfo[MAX_POOLS] = {
    &s_miningInfoV1[0], &s_miningInfoV1[1], &s_miningInfoV1[2], &s_miningInfoV1[3],
};
static_assert(MAX_POOLS == 4, "miningInfo initializer list must be extended to match MAX_POOLS");
...
void create_jobs_task(void *pvParameters)
{
    ...
    uint32_t last_ntime[MAX_POOLS]{0};
